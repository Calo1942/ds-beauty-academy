...
CREATE TABLE matriculas (
    ...
    fecha_creacion DATETIME DEFAULT CURRENT_TIMESTAMP,
    fecha_actualizacion DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY unique_estudiante_instructor_curso (id_estudiante, id_instructor_curso),
    FOREIGN KEY (id_estudiante) REFERENCES estudiantes(id_estudiante) ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY (id_instructor_curso) REFERENCES instructor_curso(id_instructor_curso) ON DELETE RESTRICT ON UPDATE CASCADE,
    INDEX idx_instructor_curso (id_instructor_curso),
...
CREATE TABLE evaluaciones (
    ...
    fecha_creacion DATETIME DEFAULT CURRENT_TIMESTAMP,
    fecha_actualizacion DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY unique_orden_evaluacion (id_matricula, orden_evaluacion),
    FOREIGN KEY (id_matricula) REFERENCES matriculas(id_matricula) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE entregas_evaluaciones (
    ...
    fecha_creacion DATETIME DEFAULT CURRENT_TIMESTAMP,
    fecha_actualizacion DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY unique_entrega (id_evaluacion),
...
-- ==========================================
-- TRIGGERS - DISPARADORES ACTUALIZADOS
-- ==========================================

DELIMITER //

-- 1. Control de cupo en INSERT (Ajustado a la relación instructor_curso)
...
BEGIN
    ...
    SELECT id_curso INTO v_id_curso FROM instructor_curso WHERE id_instructor_curso = NEW.id_instructor_curso;
    SELECT limite_cupos INTO limite FROM cursos WHERE id_curso = v_id_curso;
    
    SELECT COUNT(*) INTO cupos_usados
    FROM matriculas m
    JOIN instructor_curso ic ON m.id_instructor_curso = ic.id_instructor_curso
    WHERE ic.id_curso = v_id_curso
      AND m.estatus_cupo IN ('Reservado', 'Confirmado');
...
-- 2. Control de cupo en UPDATE (Ajustado a la relación instructor_curso)
...
BEGIN
    ...
    IF NEW.estatus_cupo = 'Confirmado' 
       AND OLD.estatus_cupo != 'Confirmado' THEN
        ...
        JOIN instructor_curso ic ON m.id_instructor_curso = ic.id_instructor_curso
        WHERE ic.id_curso = v_id_curso
          AND m.estatus_cupo = 'Confirmado'
          AND m.id_matricula != NEW.id_matricula;
...
-- 3. Impedir eliminar cursos con matrículas vigentes (Ajustado con JOIN)
...
BEGIN
    ...
    FROM matriculas m
    JOIN instructor_curso ic ON m.id_instructor_curso = ic.id_instructor_curso
    WHERE ic.id_curso = OLD.id_curso
      AND m.estatus_cupo IN ('Reservado', 'Confirmado');
...
-- 4. Al verificar/rechazar un pago, actualizar la matrícula (Se mantiene igual, no requiere ajustes estructurales)
...
-- 5. Generar certificado automático (Ajustado a la nueva jerarquía evaluaciones -> matriculas -> instructor_curso -> cursos)
...
BEGIN
    ...
    IF NEW.estatus_entrega = 'Aprobada' AND OLD.estatus_entrega != 'Aprobada' THEN
        ...
        -- Obtener la matricula desde la evaluacion
        SELECT id_matricula INTO v_id_matricula FROM evaluaciones WHERE id_evaluacion = NEW.id_evaluacion;
        ...
        WHERE e.id_matricula = v_id_matricula
        ...
        IF total_eval = aprobadas AND existe_cert = 0 THEN
            -- Obtenemos el nombre del curso atravesando las nuevas relaciones
            SELECT c.nombre_curso INTO nombre_c 
            FROM cursos c
            JOIN instructor_curso ic ON c.id_curso = ic.id_curso
            JOIN matriculas m ON m.id_instructor_curso = ic.id_instructor_curso
            ...
            VALUES (v_id_matricula, CONCAT('Certificado de Aprobación - ', nombre_c), 'Completó satisfactoriamente todas las evaluaciones.', 'General', NULL, 'Pendiente de Emisión');
        END IF;
    END IF;
END//
